...
/**
 * This class extends the main WCF class by story archiv system specific functions.
 *
 * @author 	Jana Pape
 * @copyright	2010
 ...
 */
class SLSCore extends WCF implements PageMenuContainer, UserCPMenuContainer, UserProfileMenuContainer {
    ...
    protected function initTPL() {
        ...
        // check offline mode
        if (OFFLINE && !self::getUser()->getPermission('user.sls.canViewArchivOffline')) {
   ...
   //     self::getTPL()->assign('executeCronjobs', WCF::getCache()->get('cronjobs-' . PACKAGE_ID, 'nextExec') < TIME_NOW);
    ...
    /**
     * Loads default cache resources of story archiv system.
    ...
    public static function loadDefaultSLSCacheResources() {
        ...
        WCF::getCache()->addResource('libraryData', SLS_DIR . 'cache/cache.libraryData.php', SLS_DIR . 'lib/system/cache/CacheBuilderLibraryData.class.php', 0, 300);
        WCF::getCache()->addResource('stat', SLS_DIR . 'cache/cache.stat.php', SLS_DIR . 'lib/system/cache/CacheBuilderStat.class.php', 0, 300);
    ...
    protected static function initPageMenu() {
        ...
        if (PageMenu::getActiveMenuItem() == '')
            PageMenu::setActiveMenuItem('sls.header.menu.library');
    }
    ...
    protected function initSession() {
        // start session
        require_once(SLS_DIR . 'lib/system/session/SLSSessionFactory.class.php');
        $factory = new SLSSessionFactory();
